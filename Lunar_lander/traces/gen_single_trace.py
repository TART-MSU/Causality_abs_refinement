...
def dqn(net_name):
    ...
    if flag ==0:
        torch.save(agent.local_nn.state_dict(), net_name)
    return scores
column_namest = ['trace',"pos_x","pos_y","vel_x","vel_y","action","result"]

dft = pd.DataFrame(columns=column_namest)



for x in range(500):

    #set the initial vel_x or vel_y

    vel_x = float(-10/10)
    vel_y = float(30/10)

    i = '64_64_64_wind_0.pth'
    ...
    for j in range(500):
        ...
        log.append([x,round(state[0],2),round(state[1],2),round(state[2],2),round(state[3],2),action])
    ...
    column_names = ["trace","pos_x","pos_y","vel_x","vel_y","action"]

    df = pd.DataFrame(log, columns=column_names)
...
dft.to_csv('lunar_single_trace.csv', index=False)
